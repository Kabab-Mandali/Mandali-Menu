...
(function () {
  ...
  function publishFile() {
    // Bump the version so the new seed.js supersedes any cached copy on
    // returning customers' devices (store.js load() is version-gated).
    var data = S.get();
    data.version = Number(data.version || 0) + 1;
    S.save();
    var header = "/* Mandali published menu — generated by the admin panel on " + new Date().toLocaleString() +
      " (v" + data.version + ") */\n";
    download("seed.js", header + "window.MANDALI_SEED = " + JSON.stringify(data, null, 2) + ";\n", "application/javascript");
    toast("Published v" + data.version + " — replace assets/js/seed.js, then redeploy");
  }
